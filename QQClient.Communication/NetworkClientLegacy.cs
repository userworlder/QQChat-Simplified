...
{
    public class NetworkClientLegacy : INetworkClient
    {
        ...
        private ChatPacket WaitForResponse(string messageId, MessageType expectedType, int timeout = 10000)
        {
            ...
            try
            {
                ...
                {
                    Console.WriteLine($"等待响应超时: {expectedType}, MessageId={messageId}");
                    return null; // 改为返回 null，不抛出异常
                }
            }
            finally
            {
        ...
        public List<Group> GetGroupList()
        {
            ...
            SendPacket(packet);
            Console.WriteLine($"[GetGroupList] 发送请求，MessageId: {packet.MessageId}");
